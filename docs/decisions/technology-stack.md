...
description: The chosen runtime, libraries, and build/dev tooling for the language tutor, with rationale for what was deliberately rejected.
...
The tutor must ship as a Claude Code plugin with a portable Python core. The stack must
optimize for: zero runtime cost beyond Claude itself, deterministic golden testing,
human-editable YAML, transactional SQLite, and the smallest dependency surface a
marketplace plugin can ship. The forces below — single-user, single-writer, event-at-a-time
lifecycle, no network I/O on the hot path — drive every choice in this decision.
...
### 2026-05-22 — Phase 6 Addendum — Host Tooling
