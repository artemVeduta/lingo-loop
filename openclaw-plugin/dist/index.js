...
async function runTutor(command, payload) {
    if (command.length === 0) {
...
export default defineToolPlugin({
    ...
    tools: (tool) => [
        tool({
            ...
            description: "Assemble learner boot context (profile, preferences, focus) on the first tutor message.",
        ...
        tool({
            ...
            description: "Present and evaluate a text-only tutor exercise (reading, lesson, transcript, vocab, writing, progress).",
            parameters: Type.Object({
        ...
        tool({
            ...
            description: "Invoke the local language-tutor CLI. Binary-dependent and side-effectful; opt-in only.",
            optional: true,
            parameters: Type.Object({
                command: Type.Array(Type.String()),
                payload: Type.Optional(Type.Record(Type.String(), Type.Unknown())),
